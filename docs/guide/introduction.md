...
accepted plan, inspectors check the result, and your role is to curate intent
and resolve meaningful changes, not to babysit generation.
...
4. **Narration is not progress.** A transcript line is not enough. Progress should leave a durable artifact: a blueprint, question, decision, implementation diff, verification result, review finding, change order, or learning record.
