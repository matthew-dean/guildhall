...
export function specReviewRequiresOwnerApproval(task: Pick<Task, 'id'> & Partial<Pick<Task, 'spec' | 'structuredSpec'>>): boolean {
  ...
  if (!Object.prototype.hasOwnProperty.call(task, 'structuredSpec')) return true
  return Boolean(task.structuredSpec)
}
