...
import {
  constructionBlockerKindForEscalationReason,
...
import { renderStructuredReviewFeedback } from './review-contract.js'
import {
...
//
// This builder assembles a focused, task-scoped context block. This is
// critical for local LLMs with limited context windows — irrelevant content
// actively degrades output quality. Durable project memory comes from the
// typed effective-memory packet; legacy Markdown history is never ranked or
// injected here.
//
// Each agent receives:
//   1. Its own role description (from its system prompt)
//   2. The specific task it's working on
//   3. Typed effective memory selected from structured routing keys
...
export function resolveLikelyTaskFiles(task: Task, checkpointFilesTouched: readonly string[] = []): string[] {
  ...
  // File targets are contract data. Rendered descriptions, specs, reviewer
  // notes, and imported-source notes are deliberately not executable routing
  // inputs: changing a model's prose must not change the worker's scope.
  const rootedHints = [
    ...commandCandidates,
    ...(task.structuredSpec?.targetFiles ?? []).filter(isActionableFileCandidate),
  ]
  const localWebStarterHints: string[] = []
...
function renderRetryCoaching(input: {
  ...
  const resolvedImplementationRecovery = [...input.task.escalations]
    .reverse()
    .find((escalation) => escalation.resolvedAt && (
      escalation.recoveryCode === 'tool_path_mismatch' ||
      escalation.recoveryCode === 'target_shape_mismatch' ||
      escalation.recoveryCode === 'blueprint_tooling'
    ))
...
function renderProjectSkills(task: Task, memoryDir: string, enabled: boolean): string {
  ...
  const routingKeys = [
    ...
    task.taskKind ? `task-kind:${task.taskKind}` : '',
    task.workKind ? `work-kind:${task.workKind}` : '',
    task.semanticKind ? `semantic-kind:${task.semanticKind}` : '',
    ...(task.contractNames ?? []).map((name) => `contract:${name}`),
  ].filter(Boolean)
...
function isFrontendUiTask(task: Task): boolean {
  // This is a context-routing decision, so it must come from the structured
  // work contract. Model-authored titles/specs can use any vocabulary.
  return task.workShape === 'ui-component' || task.workShape === 'frontend-integration'
}
...
function normalizedCheckpointNextAction(_task: Task, checkpoint: Checkpoint | null): string {
  const nextAction = checkpoint?.nextPlannedAction?.trim() ?? ''
  if (!nextAction) return ''
  if (/^(?:none|null|n\/a|na|nothing)$/i.test(nextAction)) return ''
  // `nextPlannedAction` is human-readable context. The orchestrator routes
  // from Checkpoint.nextActionKind instead of attempting to interpret prose.
  return nextAction
}
...
function renderLatestCheckpoint(task: Task, checkpoint: Checkpoint | null): string {
  ...
  if (checkpoint.nextActionKind) lines.push(`- Routing intent: ${checkpoint.nextActionKind}`)
...
function renderHandoffStepHeader(input: {
  ...
  const notes = priorSteps
    .map((p, i) => ({ step: i + 1, agent: p.agent, handoff: p.handoff }))
    .filter((p) => p.handoff !== undefined)
  if (notes.length > 0) {
    ...
    for (const n of notes) {
      ...
      lines.push('')
      for (const completed of n.handoff?.completed ?? []) lines.push(`Completed: ${completed}`)
      for (const knownGap of n.handoff?.knownGaps ?? []) lines.push(`Known gap: ${knownGap}`)
      if (n.handoff?.nextFocus) lines.push(`Next focus: ${n.handoff.nextFocus}`)
    }
    lines.push('')
  }
  lines.push(
    'When you finish a non-final handoff step, include a typed `handoff` object in the self-critique structured field with `completed` and `knownGaps` arrays and an optional `nextFocus` string. Do not rely on Markdown headings or prose formatting for handoff state.',
  )
  return lines.join('\n')
}
...
export async function buildContext(
  ...
  const [progress, decisions, designSystemRaw, exploring, goal, ds, worktreeResume, checkpoint, codebaseMap, languageMapData, envManifest] = await Promise.all([
  ...
  const latestReviewerNote = [...task.notes]
    .reverse()
    .find((note) =>
  ...
  // Reviewer prose is audit display only. The worker receives revision data
  // through the same structured contract for both single-reviewer and
  // fan-out notes; a differently formatted explanation must not change its
  // executable context.
  const latestRevisionFeedback = latestReviewerNote
    ? renderStructuredReviewFeedback(latestReviewerNote.content, latestReviewerNote.structured)
    : ''
  ...
  const constructionMode = constructionModeForTask({
    status: task.status,
    blockerKind: task.escalations?.find(escalation => !escalation.resolvedAt)
      ? constructionBlockerKindForEscalationReason(
        task.escalations.find(escalation => !escalation.resolvedAt)?.reason,
      )
      : undefined,
  })
  ...
  const structuralTask = {
    ...
    ...(task.domain
      ? { domainId: task.domain.startsWith('domain:') ? task.domain : `domain:${task.domain}` }
      : {}),
  }
  ...
  return {
    taskSummary,
    // Kept as a compatibility field for context observability and callers
    // that still shape the old packet. It is intentionally never populated;
    // effectiveMemory is the sole project-memory context path.
    projectMemory: '',
