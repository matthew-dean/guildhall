<script lang="ts">
  ...
  const sourceCapabilityCatalog = $derived(detail.sourceCapabilityCatalog ?? { availability: 'unavailable', total: 0 })
  ...
  const sourceRows = $derived.by(() => {
    ...
    const catalogRow = {
      ...
      value: sourceCapabilityCatalog.availability === 'ready'
        ? countLabel(sourceCapabilityCatalog.total, 'structured capability', 'structured capabilities')
